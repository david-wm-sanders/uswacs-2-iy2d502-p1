    ...
    \begin{enumerate}[label=\roman*~\textcolor{light-gray}{|}]
      \item To build the \texttt{salapp} container from the \texttt{Dockerfile} run the \texttt{salapp/make\_salapp.sh} script \textit{(shown in Listing~\ref{lst:meth:make_salapp_sh})} with \term{sudo ./make\_salapp.sh}\\
        ...
        \begin{itemize}
          ...
          \item Having a \texttt{Dockerfile} \textit{(shown in Listing~\ref{lst:meth:Dockerfile})} that defines commands to:
            \begin{enumerate}
              \item Derive the container \term{FROM} the \texttt{python:3.7-alpine} image on the Docker Hub
              \item \term{adduser} called \texttt{salapp} with a home directory
              \item Set the \term{WORKDIR} in the container to the home directory for \texttt{salapp}
              \item Copy \texttt{salapp/requirements.txt} \textit{(shown in Listing~\ref{fcl:salapp:requirements})} into the container
              \item create venv
              \item install reqs into venv
              \item install other reqs into venv
              \item copy in app and migrations
              \item copy in boot.sh and make executable
              \item envs
              \item chown shit
              \item become salapp
              \item expose the gunicorn port to world outside container
              \item set the entrypoint to be the boot.sh script
            \end{enumerate}
        \end{itemize}
        \begin{listing}[H]
          \captionsetup{skip=\skiplistingcaptionlen}
