        ...
        \begin{itemize}
          ...
          \item Having a \texttt{Dockerfile} \textit{(shown in Listing~\ref{lst:meth:Dockerfile})} that defines commands that:
            ...
            \begin{enumerate}[label=\roman*\textasciitilde]
              ...
              \item \term{RUN venv/bin/pip install gunicorn pymysql} in order to install the WSGI HTTP server \texttt{gunicorn} and python library \texttt{pymysql} in the \texttt{venv}
